        ...
        while (try reader.readUntilDelimiterOrEof(&line_buffer, '\n')) |line| {
            ...
            if (parse_state == .stage_complete) {
                ...
                if (!self.snippet_reflection_map.contains(snippet_id)) {
                    ...
                    std.log.info("snippet id: {}, stage: {s}, uniform name: {s}, inputs: {}, outputs: {}, images: {}", .{ snippet_id, @tagName(reflection.stage), stage_name, reflection.inputs.items.len, reflection.outputs.items.len, reflection.images.items.len });
                }
